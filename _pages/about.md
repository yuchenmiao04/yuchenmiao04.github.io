---
permalink: /
title: "Yuchen Miao"
excerpt: ""
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<span class='anchor' id='about-me'></span>

I am an undergraduate student in Communication Engineering at **Northeastern University**. My research interests include **LLM agents and retrieval-augmented reasoning**, **trustworthy multimodal AI**, and **recommender systems**. I am currently seeking **PhD opportunities for Fall 2027**.

# 🔥 News

- *2026.08*: &nbsp;🎉🎉 Our paper on open-set standpoint screening was accepted by the [EMNLP 2026 Main Conference](https://2026.emnlp.org/).
- *2026.08*: &nbsp;🎉🎉 Three papers on uncertainty-aware recommendation, multimodal fake-news detection, and cold-start recommendation were accepted by [WISE 2026](https://conferences.sigappfr.org/wise2026/).
- *2026.05*: &nbsp;🏆 Received the **Finalist Award (Top 1%)** in the [COMAP Mathematical Contest in Modeling](https://www.comap.org/contests/mcm-icm).
- *2026.05*: &nbsp;🎉🎉 *R3-REC* appeared at [ICASSP 2026](https://2026.ieeeicassp.org/) in Barcelona.
- *2025*: &nbsp;🏆 Received the **National Third Prize** in the [18th Advanced Robot and Simulation Technology Competition](https://urc.ilur.org/news/385).
- *2023–2026*: &nbsp;🏆 Received the **University Third-Class Scholarship** five times.

<span class='anchor' id='publications'></span>

# 📝 Selected Publications

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">EMNLP 2026 Main</div><img src='/images/publications/emnlp-10th-juror.png' alt="Framework of The 10th Juror" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

**The “10th Juror”: Open-Set Standpoint Screening for Bureaucratic Bias Detection**

**Yuchen Miao**, Zijun Wang, Chang Han, Yurui Shi, Mingtai Zhang, and Siyang Xu

[PDF](/files/emnlp-10th-juror.pdf) · [Conference](https://2026.emnlp.org/)
</div>
</div>

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">WISE 2026</div><img src='/images/publications/uhiflow.png' alt="UHIFlow framework" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

**Anatomy of a Decision: Uncertainty-aware Hierarchical Intent Learning via Flow Matching for Multimodal Recommendation**

**Yuchen Miao**, Zijun Wang, Ke Liu, and Siyang Xu

[PDF](/files/uhiflow.pdf) · [Conference](https://conferences.sigappfr.org/wise2026/)
</div>
</div>

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">WISE 2026</div><img src='/images/publications/imex-fnd.png' alt="IMEX-FND framework" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

**IMEX-FND: A Traceable Interaction-Aware Mixture-of-Experts Framework for Multimodal Fake News Detection**

**Yuchen Miao**, Zijun Wang, Ke Liu, Peixuan Wang, and Chang Han

[PDF](/files/imex-fnd.pdf) · [Conference](https://conferences.sigappfr.org/wise2026/)
</div>
</div>

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">ICASSP 2026</div><img src='/images/publications/r3-rec.png' alt="R3-REC framework" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

**R3-REC: Reasoning-Driven Recommendation via Retrieval-Augmented LLMs over Multi-Granular Interest Signals**

**Yuchen Miao**, Mingxuan Cui, Yitong Zhu, Yu Wang, and Siyang Xu

[IEEE Xplore](https://ieeexplore.ieee.org/document/11460809/) · [Conference](https://2026.ieeeicassp.org/)
</div>
</div>

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">WISE 2026</div><img src='/images/publications/motif.png' alt="MOTIF framework" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

**MOTIF: Motivation-guided Topology Inference for Cold-start Multimodal Recommendation**

Yurui Shi$^\dagger$, **Yuchen Miao$^\dagger$**, Ximing Hu, Zijun Wang, and Chang Han

($\dagger$ means equal contribution)

[PDF](/files/motif.pdf) · [arXiv](https://arxiv.org/abs/2608.25381) · [Conference](https://conferences.sigappfr.org/wise2026/)
</div>
</div>

## Preprints

- **Universal Refinement without Interaction: Order-Optimal 1-Bit Mean Estimation.** **Yuchen Miao.** 2026. [arXiv](https://arxiv.org/abs/2607.24358)
- **Sharp Root Anti-Concentration via Projective Incidence and Ordered Root Laws.** Zijun Wang, **Yuchen Miao**, Yifan Hu, and Huanmin Liu. 2026. [arXiv](https://arxiv.org/abs/2608.01670)

<span class='anchor' id='experience'></span>

# 💻 Research & Experience

- *2025–Present*, **Hebei Key Laboratory of Marine Perception Network and Data Processing**. Research on explainable multimodal recommendation and fake-content detection using LLM reasoning, retrieval augmentation, graph learning, flow matching, and mixture-of-experts models.
- *2025–Present*, **National College Students’ Innovation and Entrepreneurship Training Program**, Project No. 202619145026. Research on explainable recommendation and fake-content detection for new-media security.
- *2025.06–2025.08*, **Information Technology Intern**, China Development Bank, Shanxi Branch.
- *2024.07–2024.09*, **Big Data Center Intern**, Huajin Coking Coal Co., Ltd.
- *2024.07–2024.10*, **Embedded Systems Development Intern**, Beijing Kedinoyi Technology Co., Ltd. (Outstanding Intern)

## Patent

**面向多模态虚假内容检测的多域混合专家解释性方法。** 发明人：**苗煜晨**、王雨、王刚、冯鑫、徐思洋。中国发明专利申请，申请号：202511816918.8；实质审查中。

<span class='anchor' id='education'></span>

# 📖 Education

- *2023–2027*, **Northeastern University**, B.Eng. in Communication Engineering, Sydney Smart Technology College.
- IELTS: 7.0.

<span class='anchor' id='awards'></span>

# 🎖 Honors and Awards

- *2026.05*, **Finalist Award (Top 1%)**, [COMAP Mathematical Contest in Modeling](https://www.comap.org/contests/mcm-icm).
- *2025*, **National Third Prize**, [18th Advanced Robot and Simulation Technology Competition](https://urc.ilur.org/news/385).
- *2023–2026*, **University Third-Class Scholarship**, awarded five times.
- *2025*, **Provincial Second Prize**, [National College Students Market Research and Analysis Competition](https://www.china-cssc.org/show-289-1700-1.html).
- *2025*, **Provincial Second Prize**, [Hebei Provincial College Students “Research Hebei” Social Survey Program](https://tuanwei.hbu.cn/contents/541/11879.html).

---

Last updated: September 2026.
